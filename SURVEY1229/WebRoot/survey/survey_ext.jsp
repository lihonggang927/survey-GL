...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var currentUserType = '<%=user.getPeopleType().getId()%>';
			var basePath = '<%=basePath%>';
			var year = '<%=user.getYear()%>';
			var windowForCloseFlag = '1';
			<%if (user.getPeopleType().getId().equals("3")) {%>
				var currentUserUnit = '<%=user.getUnit().getId()%>';
				var currentUserOffice = '<%=user.getUnit().getOffice()%>';
			<%} else {%>
			var currentUserUnit = 'null';
			var currentUserOffice = 'null';
			<%}
			}%>
			//监听浏览器关闭事件	
			window.onbeforeunload   =   function() {
	    		...
	    		if(b && window.event.clientY < 0 || window.event.altKey) { 
	        		window.event.returnValue = "您还没有完成测评，测评结果不会被保存";
	    		}
	    	} 
		...
		<script type="text/javascript" src="<%=basePath%>survey/JS/util.js"></script>
		<script type="text/javascript"
			src="<%=basePath%>survey/JS/surveyStore.js"></script>
		<script type="text/javascript" src="<%=basePath%>survey/JS/survey.js"></script>
